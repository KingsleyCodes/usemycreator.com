    ...
    <div className="bg-[#001E00] min-h-screen pt-32 pb-24 px-6 relative overflow-hidden font-sans">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="text-center mb-20">
          <span className="text-[#a3dcf3] font-black uppercase tracking-[0.4em] text-[10px] mb-4 block">Pricing Infrastructure</span>
          <h1 className="text-5xl md:text-7xl font-serif text-white mb-6">
            Invest in <span className="italic text-[#a3dcf3]">influence.</span>
          </h1>
          ...
          {models.map((tier) => (
            <div 
              ...
              className={`relative p-10 rounded-[2.5rem] flex flex-col transition-all duration-500 ${
                tier.highlight 
                ? 'bg-[#a3dcf3] text-[#001E00] scale-105 z-20 shadow-[0_0_50px_rgba(163,220,243,0.3)]' 
                : 'bg-white/5 border border-white/10 text-white hover:border-[#a3dcf3]/50'
              }`}
            >
              {tier.highlight && (
              ...
              <div className="mb-8">
                <p className={`text-[10px] font-black uppercase tracking-widest mb-2 ${tier.highlight ? 'text-[#001E00]/60' : 'text-[#a3dcf3]'}`}>
                  {tier.tagline}
                </p>
              ...
              <ul className="space-y-4 mb-10 flex-1">
                {tier.features.map((f) => (
                  <li key={f} className="flex items-center gap-3 text-sm font-bold tracking-tight">
                    <div className={`p-1 rounded-full ${tier.highlight ? 'bg-[#001E00]/10' : 'bg-[#a3dcf3]/20'}`}>
                      <Check className={`h-3 w-3 ${tier.highlight ? 'text-[#001E00]' : 'text-[#a3dcf3]'}`} />
                    </div>
                    {f}
                  </li>
                ))}
              ...
              <button 
                ...
                className={`w-full py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${
                  ...
                  : 'bg-[#a3dcf3] text-[#001E00] hover:bg-white shadow-lg'
                }`}
              >
