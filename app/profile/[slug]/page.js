...
export default function CreatorPublicProfile() {
  ...
  const handleSendRequest = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Create a new partnership request/chat entry
      await addDoc(collection(db, "chats"), {
        ...
        status: "pending", // Pending creator approval
      ...
      // ✅ FIX: Close the popup immediately after success
            ...
            <form onSubmit={handleSendRequest} className="space-y-6 flex-1">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full border-2 border-gray-50 bg-gray-50/50 p-4 rounded-2xl focus:border-[#a3dcf3] focus:bg-white outline-none font-medium text-sm transition-all resize-none"
              ...
              <div className="pt-4">
                <button 
                  disabled={submitting}
                  type="submit"
                  className="w-full bg-black text-white py-6 rounded-2xl font-black uppercase text-xs tracking-[0.2em] shadow-xl flex items-center justify-center gap-3 hover:bg-gray-800 transition-all disabled:opacity-50"
                >
                  {submitting ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Deploy Proposal <Send className="h-4 w-4 text-[#a3dcf3]" /></>}
                </button>
                <p className="text-[9px] text-gray-400 font-bold text-center mt-6 uppercase tracking-widest">Proposal becomes a binding contract upon creator acceptance.</p>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="h-40 w-40 rounded-[3rem] bg-white p-2 shadow-2xl">
            <div className="h-full w-full rounded-[2.5rem] bg-gray-100 flex items-center justify-center overflow-hidden border border-gray-100">
              {creator.profileImage ? (
                ...
                <span className="text-5xl font-black text-gray-300 uppercase">{creator.name?.[0]}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-6">
              <span className="bg-[#a3dcf3] text-black px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm">
                {creator.specialty || "Institutional Creator"}
              </span>
              <div className="flex items-center gap-1.5 text-gray-400 text-[10px] font-bold uppercase tracking-widest">
                <ShieldCheck className="h-3.5 w-3.5" /> Verified Deployment
              </div>
            </div>

            <h1 className="text-6xl font-black text-gray-900 tracking-tighter mb-6 uppercase">
              {creator.name}
            </h1>
            
            <p className="text-xl text-gray-600 leading-relaxed max-w-2xl mb-12 font-medium">
              {creator.bio || "No institutional bio provided. This creator is currently optimizing their portfolio data."}
            </p>

            <div className="mb-12">
                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 mb-6">Verified Destination Channels</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {creator.platforms?.map((platform) => (
                        <a 
                            ...
                            className="group flex items-center justify-between bg-white border border-gray-100 p-5 rounded-[2rem] shadow-sm hover:border-black hover:shadow-xl transition-all duration-300"
                        >
                            <div className="flex items-center gap-4">
                                <div className="h-10 w-10 bg-gray-50 rounded-xl flex items-center justify-center group-hover:bg-[#a3dcf3] transition-colors">
            ...
            <div className="bg-white border border-gray-100 rounded-[3rem] p-10 shadow-2xl sticky top-10 border-t-4 border-t-black">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-[#a3dcf3]" /> Partnership Hub
                </h3>
              ...
              <div className="space-y-4 mb-8">
                <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                    ...
                    <p className="text-sm font-black text-gray-900 uppercase">Accepting New Campaigns</p>
                </div>
              </div>
              
              <button 
                onClick={() => currentUser ? setIsRequesting(true) : router.push('/login')}
                className="w-full bg-black text-white py-6 rounded-[1.5rem] font-black uppercase text-xs tracking-[0.2em] mb-6 hover:bg-gray-800 transition-all flex items-center justify-center gap-3 shadow-xl hover:-translate-y-1"
              >
                Initialize Request <ArrowUpRight className="h-5 w-5 text-[#a3dcf3]" />
              </button>
              
              <p className="text-[9px] text-gray-400 font-bold text-center uppercase tracking-[0.2em] leading-loose">
                Secure enterprise connection required.<br/>Login as Business to proceed.
              </p>

              <div className="mt-10 pt-10 border-t border-gray-50 grid grid-cols-2 gap-6">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1">Avg Response</p>
                  <p className="text-xs font-black uppercase">24 Hours</p>
                </div>
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1">Region</p>
                  <p className="text-xs font-black uppercase">Global</p>
                </div>
              </div>
            </div>
          </div>
