      ...
      <main className="max-w-7xl mx-auto px-4 py-8 md:py-16">
        <div className="mb-12 space-y-4">
          <div className="flex items-center gap-2">
            <span className="h-[1px] w-8 bg-[#a3dcf3]"></span>
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#a3dcf3]">Talent Discovery</span>
          </div>
        ...
        <div className="relative max-w-2xl mb-12">
          ...
          <input 
            ...
            className="w-full bg-white border border-gray-200 pl-14 pr-6 py-5 rounded-2xl outline-none focus:border-[#a3dcf3] transition-all text-sm shadow-sm"
            value={searchTerm}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24">
            <Loader2 className="h-12 w-12 animate-spin text-[#a3dcf3] mb-4" />
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-300">Syncing Talent...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredCreators.map((creator) => (
              <div key={creator.id} className="group bg-white border border-gray-100 rounded-[2.5rem] p-8 hover:shadow-2xl hover:shadow-[#a3dcf3]/10 transition-all duration-500 flex flex-col h-full relative">
                <div className="flex items-center gap-5 mb-8">
                  <div className="h-16 w-16 rounded-2xl bg-black overflow-hidden border-2 border-white shadow-md">
                    {creator.photoURL ? (
                      ...
                      <div className="h-full w-full flex items-center justify-center text-[#a3dcf3] font-bold text-xl uppercase">
                        {creator.displayTitle.charAt(0)}
                      </div>
                    )}
                  </div>
                  <div>
                    <h3 className="font-bold text-lg text-gray-900 group-hover:text-[#a3dcf3] transition-colors">{creator.displayTitle}</h3>
                ...
                <div className="pt-6 border-t border-gray-50 flex items-center justify-between">
                  ...
                  >
                    Hire <ArrowRight className="h-3 w-3 text-[#a3dcf3]" />
                  </button>
                </div>
              </div>
            ))}
